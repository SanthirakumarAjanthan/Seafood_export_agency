    ...
    public partial class Customer : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    sqlCon.Open();
                    string query = "UPDATE customer SET custname=@catname,custemail=@supemail,custphone=@supphone,description=@description WHERE custno = @id";
        ...
        private void search()
        {
            ...
            {
                sqlCon.Open();
                SqlDataAdapter sqlDa = new SqlDataAdapter("SELECT * FROM customer WHERE CONCAT(custno,custname,custemail,custphone,description) Like '%" + textBox1.Text + "%'", sqlCon);
                sqlDa.Fill(dtbl);
        ...
        {
            Customer_Add act = new Customer_Add();
            act.Show();
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    sqlCon.Open();
                    string query = "DELETE FROM customer WHERE supno = @id";
                    SqlCommand sqlCmd = new SqlCommand(query, sqlCon);
                    sqlCmd.Parameters.AddWithValue("@id", textBox2.Text);
                    sqlCmd.ExecuteNonQuery();
                    search();
